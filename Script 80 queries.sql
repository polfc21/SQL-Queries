...
SELECT EMPNO, ENAME, JOB, MGR, date_format(HIREDATE,'%d/%m/%y') as HIREDATE, SAL, ifnull(COMM,'') AS COMM, DEPTNO FROM EMPLEADOS;
...
---------- ---------- --------- ---------- -------- ---------- ---------- ----------
      ...
      7934 MILLER     CLERK           7782 23/01/82       1300                    10
4 filas seleccionadas.*/
SELECT EMPNO, ENAME, JOB, MGR, date_format(HIREDATE,'%d/%m/%y') as HIREDATE, 
SAL, ifnull(COMM,'') AS COMM, DEPTNO FROM EMPLEADOS WHERE JOB LIKE 'CLERK';
...
4 filas seleccionadas.*/
SELECT EMPNO, ENAME, JOB, MGR, date_format(HIREDATE,'%d/%m/%y') as HIREDATE, 
SAL, ifnull(COMM,'') AS COMM, DEPTNO FROM EMPLEADOS WHERE JOB LIKE 'CLERK' ORDER BY ENAME;
...
4 filas seleccionadas.*/
SELECT EMPNO, ENAME, JOB, MGR, date_format(HIREDATE,'%d/%m/%y') as HIREDATE, 
SAL, ifnull(COMM,'') AS COMM, DEPTNO FROM EMPLEADOS WHERE JOB LIKE 'CLERK' ORDER BY DEPTNO DESC;
...
6 filas seleccionadas.*/
SELECT SAL, ifnull(COMM,'') COMMM FROM EMPLEADOS WHERE DEPTNO = 30 ORDER BY COMM;
...
4 filas seleccionadas.*/
SELECT DISTINCT COMM FROM EMPLEADOS WHERE COMM IS NOT NULL ORDER BY COMM;
...
14 filas seleccionadas.*/
SELECT distinctrow ENAME, ifnull(COMM, '') COMM FROM EMPLEADOS ORDER BY ENAME;
...
SELECT distinctrow ifnull(COMM,'') COMMISSION, DEPTNO FROM EMPLEADOS ORDER BY DEPTNO, COMMISSION;
...
SELECT ENAME FROM EMPLEADOS WHERE COMM IS NULL OR COMM <= SAL*0.25;
...
12 filas seleccionadas.*/
SELECT EMPNO, SAL, IFNULL(COMM,'') COMM FROM EMPLEADOS WHERE EMPNO > 7500;
...
SELECT EMPNO, ENAME, JOB, MGR, date_format(HIREDATE,'%d/%m/%y') as HIREDATE, 
SAL, ifnull(COMM,'') AS COMM, DEPTNO FROM EMPLEADOS WHERE ENAME > 'J';
...
SELECT SAL, COMM, SAL+COMM AS SALARIO_TOTAL FROM EMPLEADOS WHERE COMM IS NOT NULL;
...
SELECT SAL, IFNULL(COMM, '') COMM, IFNULL(SAL+COMM,'') AS SALARIO_TOTAL FROM EMPLEADOS WHERE COMM IS NULL;
...
4 filas seleccionadas.*/
SELECT ENAME FROM EMPLEADOS WHERE MGR = 7698 AND SAL > 1000;
...
14 filas seleccionadas.*/
SELECT DISTINCT IFNULL(COMM/(SAL+COMM) * 100,'') AS PORCENTAJE FROM EMPLEADOS ORDER BY PORCENTAJE;
...
2 filas seleccionadas.*/
SELECT DNAME FROM DEPARTAMENTOS WHERE DNAME != 'RESEARCH' AND DNAME != 'SALES' ORDER BY LOC;
...
2 filas seleccionadas.*/
SELECT ENAME, DEPTNO FROM EMPLEADOS WHERE DEPTNO != 10 AND JOB LIKE 'CLERK' AND SAL > 800 ORDER BY HIREDATE;
...
SELECT ENAME, SAL/COMM COCIENTE FROM EMPLEADOS WHERE COMM IS NOT NULL AND COMM != 0 ORDER BY ENAME;
...
SELECT EMPNO, ENAME, JOB, MGR, date_format(HIREDATE,'%d/%m/%y') as HIREDATE, 
SAL, ifnull(COMM,'') AS COMM, DEPTNO FROM EMPLEADOS WHERE LENGTH(ENAME) = 5;
...
4 filas seleccionadas.*/
SELECT EMPNO, ENAME, JOB, MGR, date_format(HIREDATE,'%d/%m/%y') as HIREDATE, 
SAL, ifnull(COMM,'') AS COMM, DEPTNO FROM EMPLEADOS WHERE (ENAME LIKE 'A%' AND SAL > 1000) OR ((COMM IS NOT NULL AND COMM != 0) AND DEPTNO = 30);
...
14 filas seleccionadas.*/
SELECT ENAME, SAL, IFNULL(SAL+COMM,SAL) AS SALARIO_TOTAL FROM EMPLEADOS ORDER BY SAL, SALARIO_TOTAL;
...
1 fila seleccionada.*/
SELECT ENAME, JOB FROM EMPLEADOS WHERE ENAME LIKE 'A%' AND JOB LIKE '%MAN';
...
1 fila seleccionada.*/
SELECT ENAME, JOB FROM EMPLEADOS WHERE ENAME LIKE 'A%' GROUP BY ENAME, JOB HAVING JOB LIKE 'SALESMAN';
...
14 filas seleccionadas.*/
SELECT ENAME, 
CASE WHEN COMM = 0 THEN 'NO'
     WHEN COMM IS NULL THEN 'NOSESABE'
     ELSE 'SI'
END AS COMM, IFNULL(SAL+COMM,SAL) AS SALARIO_TOTAL, ROUND(IFNULL((SAL+COMM)*1.6, SAL*1.6)) AS SALARIO_DEL_PROXIMO_A�O,
ROUND(IFNULL(((SAL+COMM)*1.6)*1.7, (SAL*1.6)*1.7)) AS SALARIO_DEL_PROXIMO_A�O FROM EMPLEADOS ORDER BY ENAME;
...
6 filas seleccionadas.*/
SELECT EMPNO, ENAME, JOB, MGR, date_format(HIREDATE,'%d/%m/%y') as HIREDATE, 
SAL, ifnull(COMM,'') AS COMM, DEPTNO FROM EMPLEADOS WHERE EMPNO IN (7844, 7900, 7521, 7782, 7934, 7678, 7369) AND EMPNO NOT IN (7902, 7839, 7499, 7878);
...
14 filas seleccionadas.*/
SELECT EMPNO, ENAME, JOB, MGR, date_format(HIREDATE,'%d/%m/%y') as HIREDATE, 
SAL, ifnull(COMM,'') AS COMM, DEPTNO FROM EMPLEADOS ORDER BY DEPTNO, EMPNO DESC;
...
4 filas seleccionadas.*/
SELECT ENAME FROM EMPLEADOS WHERE EMPNO IN (SELECT EMPNO FROM EMPLEADOS WHERE MGR > EMPNO)
...
8 filas seleccionadas.*/
SELECT EMPNO, ENAME, JOB, MGR, date_format(HIREDATE,'%d/%m/%y') as HIREDATE, 
SAL, ifnull(COMM,'') AS COMM, DEPTNO FROM EMPLEADOS WHERE SAL > (SELECT SAL FROM EMPLEADOS WHERE EMPNO = 7934) ORDER BY SAL;
...
1 fila seleccionada.*/
SELECT MAX(SAL) AS SAL_MAS_ALTO, MIN(SAL) AS SAL_MAS_BAJO, MAX(SAL)-MIN(SAL) AS DIFERENCIA FROM EMPLEADOS;
...
2 filas seleccionadas.*/
SELECT ENAME, SAL FROM EMPLEADOS WHERE SAL = (SELECT MAX(SAL) FROM EMPLEADOS) OR SAL = (SELECT MIN(SAL) FROM EMPLEADOS) ORDER BY ENAME;
...
2 filas seleccionadas.*/
SELECT DNAME, AVG(EMPLEADOS.SAL) FROM DEPARTAMENTOS JOIN EMPLEADOS ON DEPARTAMENTOS.DEPTNO = EMPLEADOS.DEPTNO
WHERE DEPARTAMENTOS.DEPTNO IN (SELECT DEPTNO FROM EMPLEADOS GROUP BY DEPTNO HAVING MIN(SAL) > 900) GROUP BY DNAME;
...
14 filas seleccionadas.*/
SELECT ENAME FROM EMPLEADOS E JOIN DEPARTAMENTOS D ON E.DEPTNO = D.DEPTNO
WHERE length(LOC) > 5 ORDER BY LOC DESC, ENAME;
...
4 filas seleccionadas.*/
SELECT ENAME FROM EMPLEADOS WHERE SAL IN (SELECT MAX(SAL) FROM EMPLEADOS GROUP BY DEPTNO);
...
1 fila seleccionada.*/
SELECT COUNT(DISTINCT JOB) NUM_JOB, COUNT(DISTINCT ENAME) NUM_EMP, COUNT(DISTINCT SAL) NUM_SAL, SUM(SAL) FROM EMPLEADOS WHERE DEPTNO = 30;
...
1 fila seleccionada.*/
SELECT COUNT(*) FROM EMPLEADOS WHERE COMM IS NOT NULL AND COMM != 0;
...
1 fila seleccionada.*/
SELECT COUNT(*) NUM_EMP FROM EMPLEADOS WHERE DEPTNO = 20;
...
2 filas seleccionadas.*/
SELECT DEPTNO, COUNT(ENAME) NUM_EMP FROM EMPLEADOS GROUP BY DEPTNO HAVING NUM_EMP > 3;
...
2 filas seleccionadas.*/
SELECT ENAME FROM EMPLEADOS WHERE DEPTNO = 10 AND JOB IN (SELECT JOB FROM EMPLEADOS WHERE DEPTNO = (SELECT DEPTNO FROM DEPARTAMENTOS WHERE DNAME LIKE 'SALES'));
...
6 filas seleccionadas.*/
SELECT ENAME FROM EMPLEADOS WHERE EMPNO IN (SELECT MGR FROM EMPLEADOS) ORDER BY ENAME DESC;
...
FROM EMPLEADOS WHERE JOB IN (SELECT JOB FROM EMPLEADOS WHERE DEPTNO IN (SELECT DEPTNO FROM DEPARTAMENTOS WHERE LOC LIKE 'CHICAGO')) ORDER BY JOB;
...
5 filas seleccionadas.*/
SELECT JOB, COUNT(*) FROM EMPLEADOS GROUP BY JOB ORDER BY JOB;
...
3 filas seleccionadas.*/
SELECT DNAME, SUM(SAL) FROM DEPARTAMENTOS D JOIN EMPLEADOS E ON D.DEPTNO = E.DEPTNO GROUP BY DNAME ORDER BY DNAME;
...
1 fila seleccionada.*/
SELECT DNAME FROM DEPARTAMENTOS D JOIN EMPLEADOS E WHERE D.DEPTNO NOT IN (SELECT DEPTNO FROM EMPLEADOS GROUP BY DEPTNO) GROUP BY DNAME;
...
8 filas seleccionadas.*/
SELECT ENAME FROM EMPLEADOS WHERE EMPNO NOT IN (SELECT MGR FROM EMPLEADOS WHERE MGR IS NOT NULL);
...
3 filas seleccionadas.*/
SELECT DNAME, AVG(SAL), COUNT(*) FROM DEPARTAMENTOS D JOIN EMPLEADOS E ON D.DEPTNO = E.DEPTNO GROUP BY DNAME;
...
6 filas seleccionadas.*/
SELECT ENAME, COMM FROM EMPLEADOS WHERE DEPTNO = 30 ORDER BY COMM DESC;
...
8 filas seleccionadas.*/
SELECT ENAME FROM EMPLEADOS WHERE DEPTNO IN (SELECT DEPTNO FROM DEPARTAMENTOS WHERE LOC LIKE 'DALLAS' OR LOC LIKE 'NEW YORK');
...
14 filas seleccionadas.*/
SELECT E.ENAME, IFNULL(J.ENAME,'') FROM EMPLEADOS E LEFT JOIN EMPLEADOS J ON J.EMPNO = E.MGR ORDER BY J.ENAME;
...
4 filas seleccionadas.*/
SELECT ENAME, SAL, DNAME FROM EMPLEADOS JOIN DEPARTAMENTOS ON EMPLEADOS.DEPTNO = DEPARTAMENTOS.DEPTNO WHERE SAL IN (SELECT MAX(SAL) FROM EMPLEADOS GROUP BY DEPTNO) ORDER BY SAL;
...
7 filas seleccionadas.*/
SELECT J.ENAME, J.EMPNO, COUNT(E.EMPNO) NUM_EMPLEADOS FROM EMPLEADOS E LEFT JOIN EMPLEADOS J ON J.EMPNO = E.MGR GROUP BY J.ENAME, J.EMPNO;
...
1 fila seleccionada.*/
SELECT DNAME, SUM(SAL) FROM EMPLEADOS E JOIN DEPARTAMENTOS D ON E.DEPTNO = D.DEPTNO GROUP BY D.DEPTNO ORDER BY SUM(SAL) DESC LIMIT 1;
...
3 filas seleccionadas.*/
SELECT EMPNO, ENAME, JOB, MGR, date_format(HIREDATE,'%d/%m/%y') as HIREDATE, SAL, ifnull(COMM,'') AS COMM, DEPTNO 
FROM EMPLEADOS WHERE SAL IN (SELECT * FROM (SELECT SAL FROM EMPLEADOS ORDER BY SAL DESC LIMIT 2) AS T) ORDER BY SAL DESC;
...
2 filas seleccionadas.*/
SELECT LOC, COUNT(ENAME) AS NUMERO_EMPLEADOS FROM DEPARTAMENTOS D JOIN EMPLEADOS E ON D.DEPTNO = E.DEPTNO
WHERE LOC NOT IN (SELECT LOC FROM DEPARTAMENTOS D LEFT JOIN EMPLEADOS E ON D.DEPTNO = E.DEPTNO GROUP BY LOC, D.DEPTNO HAVING COUNT(ENAME) = 0) 
GROUP BY LOC HAVING NUMERO_EMPLEADOS > 4;
